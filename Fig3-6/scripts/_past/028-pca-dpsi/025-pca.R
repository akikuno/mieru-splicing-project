...
mat_spliced <- df_spliced %>%
    mutate(event_target = str_c(event, target_symbol, sep = "_")) %>%
    select(ko_symbol, event_target, dpsi) %>%
    group_by(ko_symbol, event_target) %>%
    summarise(dpsi = mean(dpsi)) %>%
    pivot_wider(names_from = ko_symbol, values_from = dpsi, values_fill = 0) %>%
    filter(!is.na(event_target)) %>%
    column_to_rownames("event_target")

pca_spliced <- mat_spliced %>%
    prcomp(center = TRUE, scale = TRUE)

pca_spliced$rotation %>%
    as.data.frame() %>%
    rownames_to_column("event_target") %>%
    ggplot(aes(x = PC1, y = PC2, color = event_target)) +
    geom_point(size = 5) +
    theme_minimal() +
    theme(text = element_text(family = "Arial", size = 20)) +
    labs(title = "PCA by dpsi")
